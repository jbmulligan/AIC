...
function applyFilters() {
  ...
  const showUnlabelledOnly = document.getElementById('unlabelledOnly').checked;

  filteredClips = allAnnotations.filter(clip => {
    // 1. Get the description string (fallback to 'label' column if present)
    const labelText = (clip.description || clip.label || '').trim();

    // Check if the clip is considered unlabelled
    const isUnlabelled = labelText === '' || labelText.toLowerCase() === 'unlabelled';

    // 2. Evaluate 'Unlabelled Only' condition
    if (showUnlabelledOnly && !isUnlabelled) {
      return false;
    }

    // 3. Evaluate Text Search condition (skip search check if searching for unlabelled)
    if (query !== '') {
  ...
  // Reset navigation pointer to the first item in the new filtered set
  activeIndex = 0;
  updateSearchUI();

  if (filteredClips.length > 0) {
console.log("calling loadCurrentClip");
    loadCurrentClip();
  } else {
...
function updateSearchUI() {
  ...
  const masterIndex = allAnnotations.findIndex(c => c.filename === currentClip.clipFilename) + 1;

  // 3. Update the UI index widget / counter display
  // e.g., "Clip 14 of 282 (Filter result 2 of 5)"
  const indexWidget = document.getElementById('clipIndexDisplay'); // adjust ID to match your HTML
  if (indexWidget) {
  ...
  document.getElementById('clipLabelDisplay').textContent = currentClip.description || "Unlabelled";
}
/**
 * Loads the active clip from filteredClips.
 */
	  /*
function loadCurrentClip() {
...
console.log("loadCurrentClip: activeIndex = "+activeIndex);
  const currentClip = filteredClips[activeIndex];
console.log("loadCurrentClip: currentClip = "+JSON.stringify(currentClip));
  const videoUrl = BASE_URL + currentClip.clipFilename;
console.log("loadCurrentClip: videoUrl = "+videoUrl);

  const videoPlayer = document.getElementById('videoPlayer');
  videoPlayer.src = videoUrl;
  videoPlayer.load();
  videoPlayer.play().catch(err => {
    console.log("Auto-play prevented or paused:", err);
  });

  // Display clip description or placeholder in UI
  const displayLabel = currentClip.description || "Unlabelled";
  document.getElementById('clipLabelDisplay').textContent = displayLabel;
}
*/
...
function showNoResultsMessage() {
  ...
  videoPlayer.pause();
  videoPlayer.src = "";
  document.getElementById('clipLabelDisplay').textContent = "No matching clips found";
}
...
console.log("loadClip: currentIndex = "+currentIndex);
  ...
  /*
     function onCsvLoaded(parsedData) {
  allAnnotations = parsedData;
  ...
  if (unlabelledCheckbox) unlabelledCheckbox.checked = unlabelledParam;
  ...
  // If a specific ?clip=N was in the URL, set activeIndex to point to that item within filteredClips
  if (targetClipNum && targetClipNum > 0 && targetClipNum <= allAnnotations.length) {
    const targetClip = allAnnotations[targetClipNum - 1];
    const matchIndex = filteredClips.findIndex(c => c.filename === targetClip.filename);
